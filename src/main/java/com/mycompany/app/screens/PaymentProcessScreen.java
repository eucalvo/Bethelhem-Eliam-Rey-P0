...
public class PaymentProcessScreen implements IScreen {
    ...
    public void start(Scanner scan) {
        ...
        if(cardName.equalsIgnoreCase("x"))
            break exit;
        do {
            System.out.print("\n Enter Card Number: ");
        ...
        do{
            ...
            if(cardYearExpiration.equalsIgnoreCase("x"))
                break exit;
            System.out.print("\n Enter month expiration date: ");
            cardMonthExpiration = scan.nextLine();
            if(cardMonthExpiration.equalsIgnoreCase("x"))
                break exit;
        }while(!paymentProcessService.cardExpirationDateValid(cardMonthExpiration, cardYearExpiration));
        if(paymentProcessService.paymentProcessedSuccessfully(cardName, cardNumber, cardYearExpiration, cardMonthExpiration)){
